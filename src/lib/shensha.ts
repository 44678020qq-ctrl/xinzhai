...
export type CheckResult = {position: string; description: string; warning?: string} | Array<{position: string; description: string; warning?: string}> | null;


export function getSanHeGroup(zhi: string): string | null {
...
export const SHEN_SHA_TABLE: Record<string, Array<{name: string; check: (bazi: any) => CheckResult}>> = {
  // ============ 贵人星系 ============
  '天乙贵人': [{
    name: '天乙贵人',
    check: (b) => {
...
// 调候简表（10日主×12月令核心需求）
export function getTiaoHou(dayGan: string, monthZhi: string): { coreNeed: string[]; reason: string; avoid: string[] } {
  // 简化版：只取月令地支判断季节
...
export function calcShenSha(bazi: any): Array<{name: string; position: string | string[]; description: string; warning?: string}> {
